...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        samples = samples.to(device)        
        targets = [{k: to_device(v, device) if isinstance(v, (torch.Tensor, list, dict)) else v for k, v in t.items()} for t in targets]
        ...
        res = {(target['image_id'].item() if hasattr(target['image_id'], 'item') else target['image_id']): output for target, output in zip(targets, results)}
        ...
        if panoptic_evaluator is not None:
            ...
            for i, target in enumerate(targets):
                image_id = target["image_id"].item() if hasattr(target["image_id"], "item") else target["image_id"]
                file_name = f"{image_id:012d}.png" if isinstance(image_id, int) else f"{image_id}.png"
