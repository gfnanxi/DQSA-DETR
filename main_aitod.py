...
def get_args_parser():
    ...
    parser.add_argument('--dataset_file', default='aitod_v2') 
    parser.add_argument('--coco_path', type=str, default='/mnt/dataset/aitod/')
...
def main(args):
    ...
    if args.onecyclelr:
        ...
        # # 2. 定义 warmup 调度器（例如前 N 个 epoch 线性从 0.001*lr 到 lr）
        # warmup_epochs = getattr(args, 'warmup_epochs', 4)  # 默认 warmup 1 个 epoch
        # warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
        #     optimizer,
        #     start_factor=0.01,      # 初始学习率 = base_lr * 0.001
        #     end_factor=1.0,          # 结束时 = base_lr * 1.0
        #     total_iters=warmup_epochs
        # )
        
        # # 3. 组合：先 warmup，再 MultiStepLR
        # lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
        #     optimizer,
        #     schedulers=[warmup_scheduler, main_scheduler],
        #     milestones=[warmup_epochs]  # 在 warmup_epochs 后切换到 main_scheduler
        # )
    else:
