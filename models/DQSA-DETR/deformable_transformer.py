...
from .DDAQ import DDAQ
from .DGFE import DGFE, MultiScaleFeature
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.DDAQ = DDAQ(cls_num=self.ccm_cls_num)
        self.DGFE = DGFE(gate_channels=256, reduction_ratio=16, num_feature_levels=self.num_feature_levels)
    ...
    def forward(self, srcs, masks, pos_embeds, dn_targets, args_dn):
        ...
        counting_output, ccm_feature = self.DDAQ(memory, spatial_shapes)
        multi_ccm_feature = self.multiscale(ccm_feature)
        cgfe_out = self.DGFE(multi_ccm_feature, memory, spatial_shapes)
        memory = cgfe_out
        _, predicted = torch.max(counting_output.detach(), 1)
