...
from .losses import SA_IA_BCE_loss

class DQSADETR(nn.Module):
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        """Classification loss using SA_IA_BCE_loss (area-aware soft BCE)"""

        assert 'pred_logits' in outputs
        assert 'pred_boxes' in outputs  # 新增：需要预测框来计算面积
        ...
        target_boxes = torch.cat([t["boxes"][J] for t, (_, J) in zip(targets, indices)], dim=0)  # (M, 4)

        # === 3. 提取正样本的预测框 ===
        src_boxes_pos = src_boxes[idx]  # (M, 4)

        # === 4. 构造 pos_idx_c: (batch_idx, src_idx, target_class) ===
        # 注意：SA_IA_BCE_loss 需要知道每个正样本对应哪个类别
        pos_idx_c = (batch_idx, src_idx, target_classes_o)

        # === 5. 调用你的损失函数 ===
        loss_ce = SA_IA_BCE_loss(
            src_logits=src_logits,
            pos_idx_c=pos_idx_c,
            src_boxes=src_boxes_pos,
        ...
        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            ...
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    ...
                    t = torch.arange(len(targets[i]['labels']), device='cuda', dtype=torch.long)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='dqsadetr')
def build_dqsadetr(args):
